 ...
 *
 * `Context` carries the per-poll `Waker` that a suspended future must store and
 * call when it becomes ready to make progress.
 *
...
class Context {
public:
    explicit Context(Rc<Waker> waker);
    virtual ~Context();
    ...
    Weak<Waker>   get_weak_waker() const;
