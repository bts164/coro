...
class CoroRecipe(ConanFile):
    ...
    def layout(self):
        ...
        if self.settings.os == "baremetal":
            ...
            self.cpp.source.build_modules = ["cmake/pico_hal_build_module.cmake"]
    ...
    def package_info(self):
        if self.settings.os == "baremetal":
            ...
            # See cmake/pico_hal_build_module.cmake: re-links hardware_dma/
            # hardware_irq onto coro::pico_hal after CMakeDeps declares it,
            # since those raw Pico SDK targets can't be expressed as a
            # normal cpp_info dependency. CMakeDeps only reads build_modules
            # off the root cpp_info — there's no per-component variant in
            # the generated *-config.cmake — so this must be set here, not
            # on cpp_info.components["pico_hal"].
            self.cpp_info.set_property(
                "cmake_build_modules", ["cmake/pico_hal_build_module.cmake"])
