...
template <ByteBuffer Buf>
JoinHandle<std::pair<std::size_t, Buf>> TcpStream::read_exact(Buf buf) {
    return with_context(*m_uv_exec,
        [](std::shared_ptr<Handle> handle, Buf buf) -> Coro<std::pair<std::size_t, Buf>> {
            auto view = std::as_writable_bytes(std::span(buf));
            std::size_t total = 0;
            while (total < view.size()) {
                ...
                ReadCtx ctx{view.subspan(total)};
                handle->handle.data = &ctx;
                ...
                if (r < 0)
                    throw std::system_error(
                        std::error_code(-r, std::system_category()), "TcpStream::read_exact");

                ssize_t nread = std::get<0>(co_await wait(ctx.result));
                handle->handle.data = nullptr;
